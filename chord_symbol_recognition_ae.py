...
import HT.chord_recognition_models as crm
import os
...
def train_HT(hp, train_data, test_data):
    ...
    n_iterations_per_epoch = int(math.ceil(n_train_sequences/hp.n_batches)) - 50
    # n_iterations_per_epoch = 2
    ...
    with tf.name_scope('model'):
        ...
        chord_change_logits, dec_input_embed, enc_weights, dec_weights, _, _ = crm.HTv2(encoded, source_mask, target_mask, slope, dropout, is_training, hp)
    ...
    with tf.name_scope('loss'):
        ...
        # Total loss
        loss = loss_cc + loss_tc + loss_ae
    valid = tf.reduce_sum(target_mask)
    summary_loss = tf.Variable([0.0, 0.0, 0.0, 0.0], trainable=False, dtype=tf.float32)
    summary_valid = tf.Variable(0, trainable=False, dtype=tf.float32)
    update_loss = tf.assign(summary_loss, summary_loss + valid * [loss, loss_cc, loss_tc,loss_ae ])
    ...
    tf.summary.scalar('Loss_ae', summary_loss[3])
    ...
    #mysaver = tf.compat.v1.train.Saver([hp.autoencoder.weights])
    # Training
    ...
    with tf.Session() as sess:
        ...
        for step in range(hp.n_training_steps):
            ...
            # print("~~~~~~~~~~~~~~~~~")
            # print(train_data['tchord'].shape)
            # print(train_data['tchord'][batch_indices[step % len(batch_indices)]])
            # print(train_data['tchord'][0])

            # print(train_data['pianoroll'].shape)
            # print(train_data['pianoroll'][batch_indices[step % len(batch_indices)]])
            # time.sleep(200)

            train_run_list = [train_op, update_valid, update_loss, update_count, loss, loss_ae, loss_cc, loss_tc, pred_cc, pred_tc, chord_mask, enc_weights, dec_weights]
            ...
            _, _, _, _, train_loss, train_pred_ae, train_loss_cc, train_loss_tc, train_pred_cc, train_pred_tc, train_chord_mask, enc_w, dec_w = sess.run(train_run_list, feed_dict=train_feed_fict)
            if step == 0:
                print(train_pred_ae.shape)
                print('*~ loss_cc %.4f, loss_tc %.4f ~*, loss_ae %.4f ~*' % (train_loss_cc, train_loss_tc, train_pred_ae))

            # Display training log & Testing
            if step > 0 and step % n_iterations_per_epoch == 0:
                #mysaver.save(sess, 'mmymymymy', global_step=0)
                ...
                #     % (step, step // n_iterations_per_epoch, train_loss[0], train_loss[1], train_loss[2], train_loss[3], train_score[0], train_score[1], train_score[2], train_score[3]))
                print('*~ loss_cc %.4f, loss_tc %.4f ~*, loss_ae %.4f ~*' % (train_loss_cc, train_loss_tc, train_pred_ae))
                ...
                if step > 0 and (test_score[0] + sq) > (best_score[0] + best_score[-1]):
                    ...
                    saver.save(sess, hp.dataset + "saved_ckpt/" +  '_' + str(hp.test_set_id) + '.ckpt')
                else:
                    in_succession += 1
